/*
 * ASSEMBLER/VERIFY_AST.c — AST semantic verification pass (skeleton).
 *
 * Validates the assembled AST: operand counts/types, register widths,
 * immediate ranges, label definitions, data declarations.
 */
#include "ASSEMBLER.h"
...
STATIC PLABEL_TABLE alloc_label_table() {
    PLABEL_TABLE t = AC_MAlloc(sizeof(LABEL_TABLE));
    if (t) AC_MEMSET(t, 0, sizeof(LABEL_TABLE));
    return t;
}
...
STATIC VOID free_label_table(PLABEL_TABLE t) {
    ...
    for (U32 i = 0; i < t->len; i++) {
        if (t->entries[i]) {
            if (t->entries[i]->name) AC_MFree(t->entries[i]->name);
            AC_MFree(t->entries[i]);
        }
    }
    AC_MFree(t);
}
...
STATIC PLABEL_ENTRY find_label(PLABEL_TABLE t, PU8 name) {
    ...
    for (U32 i = 0; i < t->len; i++) {
        if (t->entries[i] && AC_STRICMP(t->entries[i]->name, name) == 0) {
            return t->entries[i];
        }
    }
    return NULLPTR;
...
STATIC BOOL add_label(PLABEL_TABLE t, PU8 name, U32 node_index) {
    ...
    PLABEL_ENTRY e = AC_MAlloc(sizeof(LABEL_ENTRY));
    if (!e) return FALSE;
    e->name = AC_STRDUP(name);
...
STATIC BOOL is_numeric_value(PU8 data) {
    ...
    if (!*p) return FALSE;
    U32 tmp;
    if (AC_ATOI_E(p, &tmp))     return TRUE;
    if (AC_ATOI_HEX_E(p, &tmp)) return TRUE;
    if (AC_ATOI_BIN_E(p, &tmp)) return TRUE;
    if (AC_ATOI_HEX_EP(p, &tmp)) return TRUE;
    if (AC_ATOI_BIN_EP(p, &tmp)) return TRUE;
    return FALSE;
}
...
STATIC BOOL verify_symbol_ref(PU8 sym, PLABEL_TABLE labels, U32 line, U32 op_idx) {
    ...
    if (AC_STRICMP(sym, "@f") == 0 || AC_STRICMP(sym, "@b") == 0)
...
STATIC BOOL verify_instruction(PASM_NODE node, PLABEL_TABLE labels) {
    ...
    if (!tbl) {
        AC_PRINTF("[AS VERIFY] Line %u: Instruction has no mnemonic table entry\n", node->line);
        return FALSE;
    }

    /* ── Operand count ────────────────────────────────────────────────────── */
    if (node->instr.operand_count != tbl->operand_count) {
        AC_PRINTF("[AS VERIFY] Line %u: '%s' expects %u operand(s), got %u\n",
            node->line, tbl->name,
    ...
    for (U32 i = 0; i < node->instr.operand_count; i++) {
        ...
        if (!operand_type_compatible(op->type, expected)) {
            AC_PRINTF("[AS VERIFY] Line %u: '%s' operand %u - type mismatch\n",
        ...
        case OP_NONE:
            AC_PRINTF("[AS VERIFY] Line %u: '%s' operand %u is OP_NONE\n",
                   node->line, tbl->name, i + 1);
            ok = FALSE;
            break;
        ...
        case OP_REG:
            if (op->reg == REG_NONE) {
                AC_PRINTF("[AS VERIFY] Line %u: '%s' operand %u - register is REG_NONE\n",
                       node->line, tbl->name, i + 1);
                ok = FALSE;
                break;
            }
        ...
        case OP_MEM:
            if (!op->mem_ref) {
                printf("[AS VERIFY] Line %u: '%s' operand %u - OP_MEM with null mem_ref\n",
                       node->line, tbl->name, i + 1);
                ok = FALSE;
                break;
            }
            ...
            if (op->mem_ref->base_reg  == REG_NONE &&
                ...
                printf("[AS VERIFY] Line %u: '%s' operand %u - memory has no base, index, or symbol\n",
                       node->line, tbl->name, i + 1);
                ok = FALSE;
                break;
            }
            ...
            if (op->size != SZ_NONE && tbl->size != SZ_NONE
                && op->size != tbl->size) {
                printf("[AS VERIFY] Line %u: '%s' operand %u - explicit %u-bit size "
            ...
            if (op->mem_ref->scale != 1 && op->mem_ref->scale != 2 &&
                ...
                printf("[AS VERIFY] Line %u: '%s' operand %u - invalid SIB scale %d\n",
            ...
            if (op->mem_ref->segment != REG_NONE) {
                if (op->mem_ref->segment != REG_CS && op->mem_ref->segment != REG_DS &&
                    ...
                    printf("[AS VERIFY] Line %u: '%s' operand %u - invalid segment override\n",
                           node->line, tbl->name, i + 1);
                    ok = FALSE;
                }
            }
        ...
        case OP_IMM: {
            ...
            if (!imm_in_bounds(op->immediate, sz)) {
                printf("[AS VERIFY] Line %u: '%s' operand %u - immediate 0x%X out of range "
                       "for %u-bit operand\n",
        ...
        case OP_PTR:
            if (!op->mem_ref || !op->mem_ref->symbol_name) {
                AC_PRINTF("[AS VERIFY] Line %u: '%s' operand %u - OP_PTR with no symbol\n",
                       node->line, tbl->name, i + 1);
                ok = FALSE;
            } else {
                if (!verify_symbol_ref(op->mem_ref->symbol_name, labels, node->line, i))
        ...
        default:
            AC_PRINTF("[AS VERIFY] Line %u: '%s' operand %u - unknown type %u\n",
...
STATIC BOOL verify_data_variable(PASM_NODE node, ASM_AST_ARRAY *ast, U32 node_idx) {
    ...
    if (!var) {
        AC_PRINTF("[AS VERIFY] Line %u: Data variable node has null var\n", node->line);
        return FALSE;
    }

    /* ── Variable type must be valid ──────────────────────────────────────── */
    if (var->var_type == TYPE_NONE || var->var_type >= TYPE_AMOUNT) {
        AC_PRINTF("[AS VERIFY] Line %u: Invalid variable type\n", node->line);
        return FALSE;
    }
    ...
    switch (var->var_type) {
        case TYPE_BYTE:
            if (var->is_list && var->list_len == 0) {
                AC_PRINTF("[AS VERIFY] Line %u: Byte list has zero elements\n", node->line);
                return FALSE;
            }
            if (var->raw_value == NULLPTR) {
                AC_PRINTF("[AS VERIFY] Line %u: Variable has no initializer\n", node->line);
                return FALSE;
            }
            break;
        ...
        case TYPE_WORD:
            if (var->is_list && var->list_len == 0) {
                AC_PRINTF("[AS VERIFY] Line %u: List variable has zero elements\n", node->line);
                return FALSE;
            }
            if (var->raw_value == NULLPTR) {
                AC_PRINTF("[AS VERIFY] Line %u: Variable has no initializer\n", node->line);
...
STATIC BOOL verify_label(PASM_NODE node) {
    ...
    if (!node->label.name || !*node->label.name) {
        AC_PRINTF("[AS VERIFY] Line %u: Label has no name\n", node->line);
        return FALSE;
    }
...
BOOLEAN VERIFY_AST(ASM_AST_ARRAY *ast, PASM_INFO info) {
    ...
    if (!ast || ast->len == 0) {
        if (cfg->verbose) AC_PRINTF("[AS VERIFY] AST is empty or null.\n");
        return FALSE;
    }

    if (!info) {
        if (cfg->verbose) AC_PRINTF("[AS VERIFY] ASM_INFO is null.\n");
        return FALSE;
    }
    ...
    if (!labels) {
        AC_PRINTF("[AS VERIFY] Failed to allocate label table\n");
        return FALSE;
    }
        ...
        if (node->type == NODE_LABEL || node->type == NODE_LOCAL_LABEL) {
            ...
            if (node->type == NODE_LOCAL_LABEL && check_name && scope
                && (  (check_name[0] == '@' && check_name[1] == '@')
                    || check_name[0] == '.')) {
                AC_MEMSET(scoped, 0, sizeof(scoped));
                AC_STRNCPY(scoped, scope, BUF_SZ - 1);
                AC_STRNCAT(scoped, ".", BUF_SZ - AC_STRLEN(scoped) - 1);
                AC_STRNCAT(scoped, check_name, BUF_SZ - AC_STRLEN(scoped) - 1);
                check_name = scoped;
            }
            if (find_label(labels, check_name)) {
                AC_PRINTF("[AS VERIFY] Line %u: Duplicate label '%s'\n",
                       node->line, check_name);
                error_count++;
                continue;
            }
            if (!add_label(labels, check_name, i)) {
                AC_PRINTF("[AS VERIFY] Failed to add label (table full)\n");
    ...
    if (cfg->verbose) AC_PRINTF("[AS VERIFY] Collected %u labels\n", labels->len);
    ...
    for (U32 i = 0; i < ast->len; i++) {
        ...
        switch (node->type) {
            ...
            case NODE_RAW_NUM:
                ...
                break;
            default:
                AC_PRINTF("[AS VERIFY] Line %u: Unknown node type %u\n",
                       node->line, node->type);
                ok = FALSE;
                break;
        }
    ...
    if (cfg->verbose) AC_PRINTF("[AS VERIFY] All %u nodes verified OK\n", ast->len);
