 ...
 * Output format: AC_FILE_HEADER followed by code/data/rodata sections, or raw binary if no header is requested.
 */
#include "ASSEMBLER.h"
#include "../AC_FH.h"
...
STATIC ASM_PTR_ARRAY ptrs ATTRIB_DATA;
STATIC AC_FILE_HEADER h ATTRIB_DATA;
#define FIRST_PASS 0
...
STATIC VOID EMIT_IMM(FILE *f, U32 val, ASM_OPERAND_SIZE sz) {
    AC_DEBUG_PRINTF("[ASM GEN] Emitting immediate value 0x%X with size %u bits\n", val, sz);
    switch (sz) {
...
STATIC BOOL ADD_ASM_PTR(PU8 name, U32 offset, ASM_DIRECTIVE section) {
    ...
    for (U32 i = 0; i < ptrs.arr_tail; i++) {
        if (ptrs.ptrs[i].name && AC_STRICMP(ptrs.ptrs[i].name, name) == 0) {
...
STATIC U32 RESOLVE_SYMBOL(PU8 name) {
    if (!name) return 0;
    if (AC_STRCMP(name, "$")  == 0) return CURRENT_OFFSET();
...
STATIC U32 eval_atom(PU8 *pp) {
    ...
    /* $IDENT (variable byte size) — must precede plain '$' */
...
STATIC BOOL ENCODE_INSTRUCTION(FILE *f, PASM_NODE node) {
    ...
    default:
        AC_PRINTF("[ASM GEN] Line %u: Unknown encoding type %u for '%s'\n",
...
STATIC BOOL ENCODE_DATA_VAR(FILE *f, PASM_NODE node) {
    ...
    if (var->var_type == TYPE_BYTE && var->raw_value[0] == '"') {
        ...
        while (*p) {
            ...
            if (!*p) break;
            U32 num = 0;
            if (AC_ATOI_E(p, &num) || AC_ATOI_HEX_EP(p, &num) || AC_ATOI_BIN_EP(p, &num)) {
                EMIT_U8(f, (U8)num);
            }
...
STATIC PU8 MAKE_SCOPED_NAME(PU8 scope, PU8 local) {
    if (!scope || !*scope) return AC_STRDUP(local);          /* no scope yet, keep as-is */
    /* Build "scope.local" */
    U8 buf[BUF_SZ];
    AC_MEMSET(buf, 0, sizeof(buf));
    AC_STRNCPY(buf, scope, BUF_SZ - 1);
    AC_STRNCAT(buf, ".", BUF_SZ - AC_STRLEN(buf) - 1);
    AC_STRNCAT(buf, local, BUF_SZ - AC_STRLEN(buf) - 1);
...
STATIC VOID RESOLVE_LOCAL_REFS(ASM_AST_ARRAY *ast) {
    for (U32 i = 0; i < ast->len; i++) {
        ...
        for (U32 j = 0; j < node->instr.operand_count; j++) {
            ...
            if (AC_STRICMP(op->mem_ref->symbol_name, "@f") == 0) {
                ...
                for (U32 k = i + 1; k < ast->len; k++) {
                    ...
                    if (n && n->type == NODE_LOCAL_LABEL && n->label.name
                        && AC_STRSTR(n->label.name, "@@")) {
                        AC_MFree(op->mem_ref->symbol_name);
                        op->mem_ref->symbol_name = AC_STRDUP(n->label.name);
                        break;
                    }
                }
            } else if (AC_STRICMP(op->mem_ref->symbol_name, "@b") == 0) {
...
STATIC BOOL GEN_EMIT_PASS(FILE *f, ASM_AST_ARRAY *ast, ASTRAC_ARGS *cfg) {
    ...
    for (U32 i = 0; i < ast->len; i++) {
        ...
        case NODE_ORG:
            ...
            if (cfg->verbose && f)
                AC_PRINTF("[ASM GEN] .org 0x%X at line %u\n",
        ...
        case NODE_SECTION:
            switch (node->dir.section) {
                case DIR_CODE_TYPE_32:
                    ...
                    if (cfg->verbose && f)
                        AC_PRINTF("[ASM GEN] .use32 at line %u\n", node->line);
                    break;
                case DIR_CODE_TYPE_16:
                    ptrs.code_type = DIR_CODE_TYPE_16;
                    if (cfg->verbose && f)
                        AC_PRINTF("[ASM GEN] .use16 at line %u\n", node->line);
                    break;
                default:
                    ptrs.current_section = node->dir.section;
                    if (cfg->verbose && f)
                        AC_PRINTF("[ASM GEN] Section switch at line %u\n", node->line);
                    break;
            }
            break;
        ...
        case NODE_LOCAL_LABEL:
            ...
            if (cfg->verbose && f)
                AC_PRINTF("[ASM GEN] Label '%s' at offset 0x%X\n",
        ...
        case NODE_DATA_VAR:
            if (node->data.var) {
                ...
                if (!ENCODE_DATA_VAR(f, node)) {
                    AC_PRINTF("[ASM GEN] Line %u: Failed to encode variable '%s'\n",
                           node->line,
...
BOOLEAN GEN_BINARY(ASM_AST_ARRAY *ast, PASM_INFO info) {
    ...
    if (!ast || ast->len == 0) {
        if (cfg->verbose) AC_PRINTF("[ASM GEN] AST is empty or null.\n");
        return FALSE;
    }
    if (!info) {
        if (cfg->verbose) AC_PRINTF("[ASM GEN] ASM_INFO is null.\n");
        return FALSE;
    }
