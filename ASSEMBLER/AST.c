...
STATIC const ASM_MNEMONIC_TABLE *RESOLVE_MNEMONIC(
    ...
    for (U32 i = 0; i < tbl_len; i++) {
        ...
        /* ── Name must match ──────────────────────────────────────── */
        if (!tbl->name || AC_STRICMP(name, tbl->name) != 0) continue;
...
BOOL TOK_EXPECT(TOK_CURSOR *cur, ASM_TOKEN_TYPE type, PU8 ctx) {
    ...
    PASM_TOK t = TOK_PEEK(cur);
    AC_PRINTF("[AST] Expected %s but got '%s' at L%u C%u (%s)\n",
        TOKEN_TYPE_STR(type),
        t ? t->txt : "EOF",
        t ? t->line : 0,
        t ? t->col  : 0,
...
STATIC PASM_NODE ALLOC_NODE(ASM_NODE_TYPE type, PASM_TOK tok) {
    PASM_NODE n = AC_MAlloc(sizeof(ASM_NODE));
    if (!n) return NULLPTR;
    AC_MEMSET(n, 0, sizeof(ASM_NODE));
    n->type = type;
...
STATIC PASM_NODE PARSE_SECTION(TOK_CURSOR *cur) {
    /* The lexer already consumed the leading dot — current token IS the directive. */
    PASM_TOK dir_tok = TOK_PEEK(cur);
    if (!dir_tok || dir_tok->type != TOK_DIRECTIVE) return NULLPTR;
    TOK_ADVANCE(cur);   /* consume directive token */

    /* ── .org <number> ───────────────────────────────────────────────── */
    if (dir_tok->directive == DIR_ORG) {
        PASM_TOK val_tok = TOK_PEEK(cur);
        if (!val_tok || val_tok->type != TOK_NUMBER) {
            AC_PRINTF("[AST] Expected number after '.org' at L%u\n", dir_tok->line);
            return NULLPTR;
        }
        TOK_ADVANCE(cur);
    ...
    /* ── .data / .rodata / .code  (optional type keyword) ─────────────── */
    if (dir_tok->directive == DIR_DATA ||
        dir_tok->directive == DIR_RODATA ||
        dir_tok->directive == DIR_CODE
    ) {
        /* consume optional type keyword (e.g. DWORD, BYTE …) */
    ...
    /* ── .use16 / .use32 ───────────────────────────────────────────── */
    if (dir_tok->directive == DIR_CODE_TYPE_16 || dir_tok->directive == DIR_CODE_TYPE_32) {
        ast_code_mode = dir_tok->directive;
        PASM_NODE n = ALLOC_NODE(NODE_SECTION, dir_tok);
        if (!n) return NULLPTR;
        n->dir.section = dir_tok->directive;
        n->type = NODE_SECTION;
    ...
    if (dir_tok->directive == DIR_TIMES) {
        ...
        while (!TOK_AT_END(cur) && !TOK_MATCH(cur, TOK_EOL)) {
            ...
            if (expr_len + tlen < BUF_SZ) {
                AC_STRNCPY(expr_buf + expr_len, t->txt, BUF_SZ - expr_len - 1);
                expr_len += tlen;
            }
            expr_first = FALSE;
        }
...
STATIC PU8 COLLECT_VALUE_TEXT(TOK_CURSOR *cur) {
    ...
    while (!TOK_AT_END(cur)) {
        ...
        if (len + tlen < BUF_SZ) {
            AC_STRNCPY(buf + len, t->txt, BUF_SZ - len - 1);
            len += tlen;
        }
        first = FALSE;
    }
...
STATIC BOOL PARSE_OPERAND(TOK_CURSOR *cur, ASM_OPERAND *out) {
        ...
        if (r1 && r1->type == TOK_REGISTER) {
            ...
            if (maybe_colon && maybe_colon->type == TOK_SYMBOL
                ...
                if (r2 && r2->type == TOK_REGISTER) {
                    ...
                    TOK_ADVANCE(cur);
                    mem->symbol_name = AC_STRDUP(r2->txt);
                }
            } else {
                mem->base_reg = r1->reg;
            }
            ...
            TOK_ADVANCE(cur);
            mem->symbol_name = AC_STRDUP(r1->txt);
    ...
    if (t && t->type == TOK_SYMBOL && t->symbol == SYM_AT) {
        ...
        if (next) {
            ...
            if (next->type == TOK_IDENTIFIER
                && (AC_STRICMP(next->txt, "f") == 0 || AC_STRICMP(next->txt, "b") == 0)) {
                TOK_ADVANCE(cur);   /* consume '@' */
                TOK_ADVANCE(cur);   /* consume 'f'/'b' */
                U8 ref_name[4];
                AC_STRCPY(ref_name, "@");
                AC_STRNCAT(ref_name, next->txt, 2);
                ...
                mem->segment   = REG_NONE;
                mem->scale     = 1;
                mem->symbol_name = AC_STRDUP(ref_name);
                out->type    = OP_PTR;
                out->mem_ref = mem;
                return TRUE;
            }
            ...
            if (next->type == TOK_SYMBOL && next->symbol == SYM_AT) {
                ...
                if (id && (id->type == TOK_NUMBER || id->type == TOK_IDENTIFIER)) {
                    ...
                    AC_STRCPY(ref_name, "@@");
                    AC_STRNCAT(ref_name, id->txt, BUF_SZ - 3);
    ...
    if (t && t->type == TOK_SYMBOL && t->symbol == SYM_DOT) {
        ...
        if (dot_ident && dot_ident->type == TOK_IDENTIFIER) {
            ...
            U8 ref_name[BUF_SZ] = { 0 };
            AC_STRCPY(ref_name, ".");
            AC_STRNCAT(ref_name, dot_ident->txt, BUF_SZ - 2);

            ASM_NODE_MEM *mem = AC_MAlloc(sizeof(ASM_NODE_MEM));
            if (!mem) { AC_PRINTF("[AST] Memory allocation failed\n"); return FALSE; }
            AC_MEMSET(mem, 0, sizeof(ASM_NODE_MEM));
            mem->base_reg    = REG_NONE;
            mem->index_reg   = REG_NONE;
            mem->segment     = REG_NONE;
...
STATIC PASM_NODE PARSE_INSTRUCTION(TOK_CURSOR *cur) {
    ...
    if (n->instr.operand_count == 2
        && (AC_STRICMP(mnem_tok->txt, "shl")  == 0
         || AC_STRICMP(mnem_tok->txt, "shr")  == 0
         || AC_STRICMP(mnem_tok->txt, "sar")  == 0
         || AC_STRICMP(mnem_tok->txt, "rol")  == 0
         || AC_STRICMP(mnem_tok->txt, "ror")  == 0)) {
    ...
    while (!TOK_AT_END(cur) && !TOK_MATCH(cur, TOK_EOL)) {
        ...
        if (t->type == TOK_SYMBOL && t->symbol == SYM_DOLLAR) {
            ...
            if (next_id && next_id->type == TOK_IDENTIFIER) {
                ...
                U32 nlen = AC_STRLEN(next_id->txt);
                if (val_len + nlen < BUF_SZ) {
                    AC_STRNCPY(val_buf + val_len, next_id->txt, BUF_SZ - val_len - 1);
                    val_len += nlen;
                }
                first = FALSE;
                continue;
        ...
        if (t->type == TOK_SYMBOL && t->symbol == SYM_MINUS) {
            ...
            if (next_num && next_num->type == TOK_NUMBER) {
                ...
                if (!first && val_len + 1 < BUF_SZ) val_buf[val_len++] = ' ';
                if (val_len + 1 < BUF_SZ) val_buf[val_len++] = '-';
                U32 nlen = AC_STRLEN(next_num->txt);
                if (val_len + nlen < BUF_SZ) {
                    AC_STRNCPY(val_buf + val_len, next_num->txt, BUF_SZ - val_len - 1);
                    val_len += nlen;
                }
                first = FALSE;
                continue;
...
STATIC PASM_NODE PARSE_LOCAL_LABEL(TOK_CURSOR *cur) {
    ...
    if (next && next->type == TOK_SYMBOL && next->symbol == SYM_AT) {
        ...
        PASM_TOK id = TOK_PEEK(cur);
        if (id && (id->type == TOK_NUMBER || id->type == TOK_IDENTIFIER || id->type == TOK_MNEMONIC)) {
            TOK_ADVANCE(cur);
            AC_STRNCPY(name_buf, "@@", BUF_SZ - 1);
            AC_STRNCAT(name_buf, id->txt, BUF_SZ - 3);
            name_len = AC_STRLEN(name_buf);
        }
        /* consume ':' */
        ...
        if (!n) return NULLPTR;
        n->label.name = AC_STRDUP(name_buf);
        return n;
    }
    ...
    AC_PRINTF("[AST] Unexpected '@' construct at L%u C%u\n", at1->line, at1->col);
    return NULLPTR;
}
...
ASM_AST_ARRAY *ASM_BUILD_AST(ASM_TOK_ARRAY *toks) {
    ASM_AST_ARRAY *arr = AC_MAlloc(sizeof(ASM_AST_ARRAY));
    if (!arr) return NULLPTR;
    AC_MEMSET(arr, 0, sizeof(ASM_AST_ARRAY));
    ...
    while(!TOK_AT_END(&cur)) {
        ...
        // AC_PRINTF("[AST] Processing token '%s' (%s) at L%u C%u\n",
        //     t->txt, TOKEN_TYPE_STR(t->type), t->line, t->col);

        /* ── TOK_DIRECTIVE → section / org / times ──────────────────────── */
        if (t->type == TOK_DIRECTIVE) {
            ...
            if (node) {
                if (node->type == NODE_SECTION) {
                    ...
                    } else if (new_sec == DIR_CODE || new_sec == DIR_DATA || new_sec == DIR_RODATA) {
        ...
        if (t->type == TOK_SYMBOL && t->symbol == SYM_DOT) {
            ...
            if (dot_next && dot_next->type == TOK_IDENTIFIER
                ...
                PASM_TOK dot_tok = TOK_ADVANCE(&cur);
                PASM_TOK id_tok  = TOK_ADVANCE(&cur);
                TOK_ADVANCE(&cur);
                U8 label_buf[BUF_SZ] = { 0 };
                AC_STRCPY(label_buf, ".");
                AC_STRNCAT(label_buf, id_tok->txt, BUF_SZ - 2);
                node = ALLOC_NODE(NODE_LOCAL_LABEL, dot_tok);
                if (node) {
                    node->label.name = AC_STRDUP(label_buf);
                    PUSH_NODE(arr, node);
                }
                continue;
            }
        ...
        if (t->type == TOK_IDENTIFIER) {
            ...
            if (lookahead && lookahead->type == TOK_SYMBOL
                ...
                if (node) {
                    node->label.name = AC_STRDUP(name_tok->txt);
                    PUSH_NODE(arr, node);
                }
                continue;
            }
...
/*
 * ════════════════════════════════════════════════════════════════════════════
 *  DESTROY_AST_ARR  —  deep free of the AST (uses AC_MFree)
 * ════════════════════════════════════════════════════════════════════════════
 */
    ...
    for (U32 i = 0; i < ast->len; i++) {
        ...
        switch (n->type) {
            ...
            case NODE_LOCAL_LABEL:
                if (n->label.name) AC_MFree(n->label.name);
                break;

            case NODE_DATA_VAR:
                if (n->data.var) {
                    PASM_VAR v = n->data.var;
                    if (v->name)      AC_MFree(v->name);
                    if (v->raw_value) AC_MFree(v->raw_value);
                    AC_MFree(v);
                }
                break;
            ...
            case NODE_INSTRUCTION:
                for (U32 j = 0; j < n->instr.operand_count; j++) {
                    ...
                    if ((op->type == OP_MEM || op->type == OP_PTR) && op->mem_ref) {
                        if (op->mem_ref->symbol_name) AC_MFree(op->mem_ref->symbol_name);
                        AC_MFree(op->mem_ref);
                    }
                }
                break;
            ...
            case NODE_TIMES:
                if (n->times.count_expr) AC_MFree(n->times.count_expr);
                if (n->times.fill_raw)   AC_MFree(n->times.fill_raw);
                break;
