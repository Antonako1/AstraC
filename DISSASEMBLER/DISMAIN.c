...
ASTRAC_RESULT START_DISSASEMBLER() {
    ...
    if (dot) {
        ...
        AC_MEMCPY(outfile, cfg->outfile, base_len);
        outfile[base_len] = '\0';
    } else {
        AC_STRCPY(outfile, cfg->outfile);
    }
    AC_STRCAT(outfile, ".DSM");
    ...
    AC_MFree(cfg->outfile);
    cfg->outfile = AC_STRDUP(outfile);
    AC_MFree(outfile);
    ...
    if (cfg->verbose) {
        AC_PRINTF("[DISASM] Disassembling %s -> %s\n", cfg->input_file, cfg->outfile);
    }
    ...
    if (cfg->verbose) {
        AC_PRINTF("[DISASM] Disassembly completed successfully.\n");
    }
...
STATIC const U8 *DSM_REG32[8] = {"eax","ecx","edx","ebx","esp","ebp","esi","edi"};
STATIC const U8 *DSM_REG16[8] = {"ax","cx","dx","bx","sp","bp","si","di"};
STATIC const U8 *DSM_REG8[8]  = {"al","cl","dl","bl","ah","ch","dh","bh"};

/* 16-bit ModRM r/m effective-address strings (mod != 3) */
STATIC const U8 *DSM_RM16[8] = {
    "bx+si", "bx+di", "bp+si", "bp+di", "si", "di", "bp", "bx"
};

/*
 * Append a formatted string into buf at position pos, max_size total.
 * Returns the new tail position (pos + chars written).
 */
STATIC U32 dsm_apnd(U8 *buf, U32 pos, U32 max_size, const U8 *fmt, ...) {
    va_list a;
    va_start(a, fmt);
    U32 remain = (pos < max_size) ? (max_size - pos) : 0;
    I32 written = vsnprintf((char *)(buf + pos), remain, (const char *)fmt, a);
    va_end(a);
    if (written < 0) return pos;
    return pos + (U32)written;
}
 ...
 * Returns the new pos.
 */
STATIC U32 dsm_mem_str_16(U8 *buf, U32 pos, U32 max_sz, const U8 *data, U32 *off,
                           U32 sz, U8 modrm) {
    U8 mod = (modrm >> 6) & 0x3;
    U8 rm  =  modrm       & 0x7;
    ...
    if (mod == 0 && rm == 6) {
        ...
        U16 d16 = 0;
        if (*off + 2 <= sz) { AC_MEMCPY(&d16, data + *off, 2); *off += 2; }
        pos = dsm_apnd(buf, pos, max_sz, "0x%04X", (U32)d16);
    } else {
        pos = dsm_apnd(buf, pos, max_sz, "%s", DSM_RM16[rm]);

        /* Displacement */
        if (mod == 1) {
            if (*off < sz) {
                I8 d8 = (I8)data[(*off)++];
                if (d8 < 0) pos = dsm_apnd(buf, pos, max_sz, "-0x%02X", (U32)(-(I32)d8));
                else        pos = dsm_apnd(buf, pos, max_sz, "+0x%02X", (U32)d8);
            }
        } else if (mod == 2) {
            if (*off + 2 <= sz) {
                I16 d16s = 0;
                AC_MEMCPY(&d16s, data + *off, 2); *off += 2;
                if (d16s < 0) pos = dsm_apnd(buf, pos, max_sz, "-0x%04X", (U32)(-(I32)d16s));
                else          pos = dsm_apnd(buf, pos, max_sz, "+0x%04X", (U32)d16s);
            }
        }
    }
 ...
 * Returns the new pos.
 */
STATIC U32 dsm_mem_str(U8 *buf, U32 pos, U32 max_sz, const U8 *data, U32 *off,
                        U32 sz, U8 modrm) {
    U8 mod = (modrm >> 6) & 0x3;
    U8 rm  =  modrm       & 0x7;
    ...
    if (rm == 4 && mod != 3) {
        ...
        if (base == 5 && mod == 0) {
            ...
            U32 d32 = 0;
            if (*off + 4 <= sz) { AC_MEMCPY(&d32, data + *off, 4); *off += 4; }
            pos = dsm_apnd(buf, pos, max_sz, "0x%08X", d32);
        } else {
            pos = dsm_apnd(buf, pos, max_sz, "%s", DSM_REG32[base]);
        }
        if (idx != 4) {
            pos = dsm_apnd(buf, pos, max_sz, "+%s*%u", DSM_REG32[idx], 1u << sc);
        }
    } else if (rm == 5 && mod == 0) {
        ...
        pos = dsm_apnd(buf, pos, max_sz, "%s", DSM_REG32[rm]);
    }
    ...
    if (mod == 1) {
        ...
        if (*off + 4 <= sz) {
            I32 d32s = 0;
            AC_MEMCPY(&d32s, data + *off, 4); *off += 4;
            if (d32s < 0) pos = dsm_apnd(buf, pos, max_sz, "-0x%08X", (U32)(-d32s));
            else          pos = dsm_apnd(buf, pos, max_sz, "+0x%08X", (U32)d32s);
        }
    }
...
BOOL DISASSEMBLE_FILE(FILE *input, FILE *output, ASTRAC_ARGS *cfg) {
    ...
    U32 size = (U32)ftell(input);
    fseek(input, 0, SEEK_SET);
    if (size == 0) return FALSE;

    U8 *file_data = (U8 *)AC_MAlloc(size);
    if (!file_data) return FALSE;
    AC_FREAD(input, file_data, size);
    const U8 *data = (const U8 *)file_data;

    U32 off  = 0;
    U32 org = cfg->org;   /* origin offset (0 unless --org specified) */
    ...
    while (off < size) {
        ...
        if (!m) {
            ...
            ilen = dsm_apnd(instr_buf, 0, DSM_IBUF_SZ, "db 0x%02X", opcode0);
        } else {
            ...
            /* Mnemonic name */
            ilen = dsm_apnd(instr_buf, 0, DSM_IBUF_SZ, "%s", m->name);
            ...
            for (U32 op = 0; op < (U32)m->operand_count; op++) {
                ...
                const U8 **regnames = (opsz == SZ_8BIT)  ? DSM_REG8  :
                ...
                switch (ot) {
                    case OP_REG: {
                        ...
                        U8 ridx = (m->encoding == ENC_REG_OPCODE)
                                  ? (opcode0 & 0x7)
                                  : reg_field;
                        ilen = dsm_apnd(instr_buf, ilen, DSM_IBUF_SZ, "%s", regnames[ridx]);
                        break;
                    }
                    case OP_MEM: {
                        if (mod_field == 3) {
                            /* register form */
                            ilen = dsm_apnd(instr_buf, ilen, DSM_IBUF_SZ, "%s", regnames[rm_field]);
                        } else if (addr16) {
                            /* 16-bit addressing */
                            ilen = dsm_mem_str_16(instr_buf, ilen, DSM_IBUF_SZ, data, &off, size, modrm);
                        } else {
                            /* 32-bit addressing */
                            ilen = dsm_mem_str(instr_buf, ilen, DSM_IBUF_SZ, data, &off, size, modrm);
                        }
                        break;
                    }
                    case OP_IMM: {
                        /* Opcodes 83h/C0h/C1h/6Bh use sign-extended imm8
                         * regardless of the instruction's operand size. */
                        U8 check_opc = is0F ? opcode1 : opcode0;
                        if (check_opc == 0x83 || check_opc == 0x6B
                            || check_opc == 0xC0 || check_opc == 0xC1)
                            opsz = SZ_8BIT;
                        ...
                        if (m->rel_type == RL_REL8) {
                            if (off < size) {
                                ...
                                ilen = dsm_apnd(instr_buf, ilen, DSM_IBUF_SZ, "0x%08X", target);
                            }
                            ...
                            if (rel_is_16) {
                                if (off + 2 <= size) {
                                    I16 rel16 = 0;
                                    AC_MEMCPY(&rel16, data + off, 2); off += 2;
                                    U32 target = (U32)((I32)off + (I32)rel16);
                                    ilen = dsm_apnd(instr_buf, ilen, DSM_IBUF_SZ, "0x%04X", target & 0xFFFF);
                                }
                            } else {
                                if (off + 4 <= size) {
                                    I32 rel32 = 0;
                                    AC_MEMCPY(&rel32, data + off, 4); off += 4;
                            ...
                            if (off < size)
                                ilen = dsm_apnd(instr_buf, ilen, DSM_IBUF_SZ, "0x%02X", data[off++]);
                        } else if (opsz == SZ_16BIT) {
                            if (off + 2 <= size) {
                                U16 imm16 = 0;
                                AC_MEMCPY(&imm16, data + off, 2); off += 2;
                                ilen = dsm_apnd(instr_buf, ilen, DSM_IBUF_SZ, "0x%04X", (U32)imm16);
                            }
                        } else {
                            if (off + 4 <= size) {
                                U32 imm32 = 0;
                                AC_MEMCPY(&imm32, data + off, 4); off += 4;
                                ilen = dsm_apnd(instr_buf, ilen, DSM_IBUF_SZ, "0x%08X", imm32);
                            }
                        }
                        break;
                    }
        ...
        /* Offset (file offset + origin) */
        ln = dsm_apnd(line, ln, sizeof(line), "%08X  ", start + org);

        /* Hex bytes (padded to 16*3 chars) */
        for (U32 i = 0; i < nbytes && i < 16; i++)
            ln = dsm_apnd(line, ln, sizeof(line), "%02X ", data[start + i]);
        ...
        line[ln++] = ' ';
        AC_MEMCPY(line + ln, instr_buf, ilen); ln += ilen;
