# AstraC Assembly language

AstraC Assembly is a NASM-like assembly language, with a few differences. It is designed to be used with the AstraC compiler and assembler, which can compile both AstraC source code and assembly code into raw binary or executable files.

Key characteristics of AstraC Assembly include:

- Intel syntax for assembly instructions, similar to NASM.
- 8-bit, 16-bit, and 32-bit instruction support, allowing for a wide range of applications.
...
1. **Preprocessing**: The assembler first preprocesses the input assembly code, handling directives, macros, and includes. This stage prepares the code for further processing.
2. **Lexical Analysis**: The preprocessed code is then tokenized into a stream of tokens, which represent the basic building blocks of the assembly language, such as mnemonics, registers, and operands.
3. **Parsing**: The token stream is parsed to create an Abstract Syntax Tree (AST), which represents the hierarchical structure of the assembly code. This stage checks for syntax errors and ensures that the code adheres to the assembly language's grammar.
4. **Semantic Analysis**: The AST is analyzed for semantic correctness, ensuring that the instructions and operands are valid and compatible. This stage also resolves labels and symbols, preparing the code for encoding.
5. **Optimization**: The assembler performs various optimizations on the AST, such as instruction selection, peephole optimizations, and constant folding. These optimizations improve the performance and efficiency of the generated machine code.
6. **Code Generation**: Finally, the optimized AST is translated into machine code, which is written to the output file in the specified format (raw binary, AC_FH).
...
Lines are terminated by newlines. The semicolon ; begins a line comment (removed during preprocessing). Backslash \ at end of line continues to the next physical line.
